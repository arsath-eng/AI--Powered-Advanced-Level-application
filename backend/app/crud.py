...
def get_theory_by_topic(db: Session, subject: str, topic: str):
    subject_obj = get_subject_by_name(db, subject)
    if not subject_obj:
        return None
    
    return db.query(models.Theory).filter(
        models.Theory.subject_id == subject_obj.id,
        (models.Theory.main_heading.ilike(f"%{topic}%")) | (models.Theory.sub_heading.ilike(f"%{topic}%")) | (models.Theory.unit.ilike(f"%{topic}%"))
    ).all()
