# backend/routers/model_papers.py
...
def create_model_paper_question(item: schemas.ModelPaperQuestionCreate, db: Session = Depends(auth.get_db)):
...
def read_model_paper_questions(skip: int = 0, limit: int = 100, db: Session = Depends(auth.get_db)):
...
def read_model_paper_question(item_id: uuid.UUID, db: Session = Depends(auth.get_db)):
...
def update_model_paper_question(item_id: uuid.UUID, item: schemas.ModelPaperQuestionUpdate, db: Session = Depends(auth.get_db)):
...
def delete_model_paper_question(item_id: uuid.UUID, db: Session = Depends(auth.get_db)):
