# backend/routers/past_papers.py
...
def create_past_paper_question(item: schemas.PastPaperQuestionCreate, db: Session = Depends(auth.get_db)):
...
def read_past_paper_questions(skip: int = 0, limit: int = 100, db: Session = Depends(auth.get_db)):
...
def read_past_paper_question(item_id: uuid.UUID, db: Session = Depends(auth.get_db)):
...
def update_past_paper_question(item_id: uuid.UUID, item: schemas.PastPaperQuestionUpdate, db: Session = Depends(auth.get_db)):
...
def delete_past_paper_question(item_id: uuid.UUID, db: Session = Depends(auth.get_db)):
...
def create_bulk_past_paper_questions(questions: list[schemas.PastPaperQuestionCreate], db: Session = Depends(auth.get_db)):
    created_questions = []
