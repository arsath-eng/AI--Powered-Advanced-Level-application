...
def update_theory(item_id: uuid.UUID, item: schemas.TheoryUpdate, db: Session = Depends(auth.get_db)):
...
def delete_theory(item_id: uuid.UUID, db: Session = Depends(auth.get_db)):
